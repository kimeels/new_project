...
print(f'-- using {ntr:d} train, {nva:d} validation and {nte:d} test slices per box')
...
for iz, z in enumerate(zs):
    for data_dir in data_dirs_z[iz]:
        ...
        # Best move might be to save as HDF5 and add the parameters as
        # attributes
        params = get_params(data_dir)
        params['z'] = z
        params['xHI'] = ix_avg

        # Randomly sample the slices to split them into
        # train/validation/test
        idxs = np.random.choice(nslc, nslc, replace=False)

        with h5py.File(os.path.join(hf_dir, 'deltaTb_slc.h5'), 'a') as hf:
            # Use sim number as g_idx
            # g_dir = pattern.search(data_dir)
            # assert(g_dir), f'Could not extract sim number from {data_dir:s} by regex'
            # g = hf.create_group(g_dir.group(1))
